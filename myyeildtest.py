...
def _makeYieldsTexTable(report, samples, entryPlots, stretch=1.5, orientation="v", align="c", yieldPrecision=1, ratioPrecision=2):
    ...
    if smp_mc:
        ...
        #cutflow_histo_Delphes = gbl.TH1F("cutflow_histo", "Cut-flow", 2, 0, 2)
        #cutflow_histo_Delphes.GetXaxis().SetTitle("Selection")
        #cutflow_histo_Delphes.GetYaxis().SetTitle("Nevent")
        #for i in range(len(colEntries_forEff)):
        #    cutflow_histo_Delphes.Fill(i, float(sel_list_array[0,i]))
